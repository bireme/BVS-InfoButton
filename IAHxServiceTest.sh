...
if [ "$Errors" != "0" ]; then
  sendemail -f appofi@bireme.org -u "IAHx - Check ERROR - `date '+%Y%m%d'`" -m "IAHx - Check ERROR" -a /tmp/$CheckFileName -t barbieri@paho.org -cc antoniov@paho.org -s esmeralda.bireme.br
