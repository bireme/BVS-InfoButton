...
class LocationOfInterest(
  ...
  assert(conceptCode.isDefined || code.isDefined || displayName.isDefined)

  val display2iAHx: Map[String, String] = ISO3166.codes1  // "Afghanistan" -> "asia",
  val code2iAHx: Map[String, String] = ISO3166.codes2     // "AFG" -> "asia",

  val iahxCode: Option[String] = conceptCode match {
    case Some("CNT") =>
      code match {
        case Some(icode) =>
          code2iAHx.get(icode) match {
            case Some(iahcode) =>
              if (iahcode.isEmpty) {
                displayName.flatMap(dn => display2iAHx.get(dn))
              } else {
                Some(iahcode)
              }
...
//println(s"*** getLoI loi=$loi cardinality=$cardinality auxSeq=$auxSeq")
    ...
    if (loiCard.isEmpty) {
      ...
      val (code, displayName) =
        if (kv._2.length == 3) (Some(kv._2), None) else (None, Some(kv._2))
      val newAuxSeq = Try (
