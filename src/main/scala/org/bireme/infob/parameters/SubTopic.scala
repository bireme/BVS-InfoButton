...
case class SubTopic(codeSystem: Option[String] = Some("2.16.840.1.113883.6.177"),
                    code: Option[String] = None,
                    displayName: Option[String] = None,
                    originalText: Option[String] = None) extends SearchParameter {
  assert ((codeSystem != null) && (code != null) &&
          (displayName != null) && (originalText != null))
  assert (codeSystem.equals("2.16.840.1.113883.6.177") ||   // MeSH
  ...
  override def toSrcExpression(conv: MeshConverter,
    ...
    code match {
      case Some(c) => conv.convert(codeSystem.get, c) match {
        case Right(co) => Some(co.map(
          cod => s"(mh:(%22${replaceSpaces(cod)}%22))").mkString("%20OR%20"))
        case Left(descr) => descr match {
          case Some(des) => Some(s"((ti:%22${des}%22)%20OR%20(ab:%22${des}%22))")
