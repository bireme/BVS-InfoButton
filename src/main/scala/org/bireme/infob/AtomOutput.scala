...
object AtomOutput {
  ...
  private def feed2Xml(feed: AtomFeed,
    ...
    root.addElement("subtitle").addAttribute("type", "text").
    ...
    feed.categories.filter(cat => (!cat.scheme.isEmpty && !cat.term.isEmpty)).
      foreach(cat => root.addElement("category").
  ...
  private def entry2Xml(entry: AtomEntry,
                        feedElem: Element): Unit = {
    require (entry != null)
    require (feedElem != null)

    val entryElem = feedElem.addElement("entry").
                             addAttribute("xml:lang", entry.lang)
    ...
    entry.updated.map(updated => entryElem.addElement("updated").addText(updated))

    entry.summary.map(summary => entryElem.addElement("summary").
                                           addAttribute("type", "text").
                                           addText(summary))

    entry.categories.filter(cat => (!cat.scheme.isEmpty && !cat.term.isEmpty)).
      foreach(cat => entryElem.addElement("category").
                               addAttribute("scheme", cat.scheme).
                               addAttribute("term", cat.term))
  ...
  private def feed2Json(feed: AtomFeed): Seq[(String, JsValue)] = {
    ...
    Seq(
      ...
      "rights" -> JsObject(List("_value" -> JsString(feed.rights._1),
                                "uri" -> JsString(feed.rights._2))),
      "icon" -> JsString(feed.icon),
      "base" -> JsString(feed.link),
      "lang" -> JsString(feed.lang),
      "title" -> JsObject(List("_value" -> JsString(feed.title),
                               "type" -> JsString("text"))),
      "subtitle" -> JsObject(List("_value" -> JsString(feed.subtitle),
                                  "type" -> JsString("text"))),
      "author" -> JsObject(List("_name" -> JsObject(List(
                                  "_value" ->JsString(feed.author._1))),
                                "uri" -> JsObject(List(
                                  "_value" ->JsString(feed.author._2))))),
  ...
  private def entry2Json(entry: AtomEntry): JsObject = {
    ...
    val seq = Seq[(String,Option[JsValue])] (
      ...
      "category" -> category,
      "title" -> entry.title.map(tit => JsObject(List("_value" -> JsString(tit),
                                                      "type" -> JsString("text")))),
      "updated" -> entry.updated.map(upd => JsObject(List("_value" -> JsString(upd)))),
      ...
      "source" -> entry.source.map(src => JsObject(List("_value" -> JsString(src))))
    )
