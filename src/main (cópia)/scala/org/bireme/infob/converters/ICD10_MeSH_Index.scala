...
object ICD10_MeSH_Index extends App {
  ...
  def createIndex(luceneIndex: String,
                  url: String,
                  cid10: String): Unit =  {
  ...
  private def filterByMeSH_ICD10(lines: collection.Iterator[String]):
    ...
    lines.foldLeft[Map[String,Set[String]]](Map()) {
      case (map, line) => {
        ...
        split.find(_.startsWith("ICD-10:")) match {
          case Some(icd10) => split.find(_.startsWith("MeSH:")) match {
            case Some(mesh) =>
              val meshCodes = mesh.substring(6).trim().split(" +")
              icd10.substring(8).trim().split(" +").foldLeft[Map[String,Set[String]]](map) {
                case (map2, icdCode) =>
  ...
  private def insertDefinitions(cid10: String,
  ...
  private def index(codes: Map[String,(String,Set[String])],
    ...
    codes.foreach {
      case (icd10, tuple) =>
        ...
        tuple._2.foreach {
          mCode => doc.add(new TextField("MeSH", mCode.toUpperCase(), Field.Store.YES))
        }
